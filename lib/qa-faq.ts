...
网站本身不能在国行安卓上走 Web Push。请打包仓库里的安卓壳（android-shell/，GitHub Actions 工作流 Build Android Shell APK，变量 SHELL_SITE_URL 填你的站点），安装 1.1.2+ 后：① **在 App 里**打开设置 → 云服务部署（Chrome 网页里配过的个人云和壳不互通）；已经建过的点「已经部署过」填项目地址和 JWT 格式 service_role（eyJ 开头）；② 聊天信息页打开「离线推送与定时消息」，创建「长时间没消息时」或「固定时间后」规则；③ 允许通知、关掉电池优化，点「测试」。通知栏若停在「请在本 App 打开设置→云服务部署」，就是壳还没拿到个人云地址：先在壳内接上个人云，并重新部署网站、重装 1.1.2+ APK。生成在个人云完成，壳负责弹系统通知。详细步骤见仓库 android-shell/README.md。

### Q: 网页能推送，安卓壳通知栏却显示「等待网页下发推送配置」？
壳和手机浏览器不是同一份数据。网页版能推，只说明 Chrome 里的个人云是好的。请打开 **App 里**的「设置 → 云服务部署」，用最上方「已经部署过？直接接上」填同一个项目地址和 service_role，不要重新走 Access Token 部署。通知应变为「已连接，等待角色消息」。仍不变则重装 1.1.2+ 壳，并确认网站已部署最新版。

### Q: 安卓壳通知栏已经「已连接」，测试推送却没有系统通知？
常见是 Realtime 频道公私不一致、或填了新版 sb_secret_ 密钥。请：① 确认允许了通知权限，且没关掉「角色消息」通知渠道；② 在 App 里用 Dashboard → API 里 **JWT 那把 service_role**（eyJ 开头）再点一次「接上已有个人云」；③ 网站更新后用 Access Token 再部署一次离线推送云函数（不要删项目）。通知栏若写「推送订阅还在登记」，等半分钟仍不变就重新接上个人云。
