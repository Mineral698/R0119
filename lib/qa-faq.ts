...
### Q: 一键部署提示「该账号下没有可用组织」？
一键部署要的是账号 Access Token（https://supabase.com/dashboard/account/tokens 的 Classic Tokens，以 sbp_ 开头），不是项目 Settings → API 里的 service_role / anon。细粒度/Scoped 令牌经常返回空组织列表。已经建过个人云的不要删项目：用同一页「已经部署过」填项目地址和 service_role key。或改用 Classic 令牌；再不行就打开组织页，把地址栏 supabase.com/dashboard/org/ 后面那段 slug 填进部署弹窗。
