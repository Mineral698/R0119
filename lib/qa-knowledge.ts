...
const QA_BASE_KNOWLEDGE_LINES = [
  ...
  "- 自部署安卓离线推送：打包 android-shell APK 1.1.2+（SHELL_SITE_URL=站点），必须在 App 里「设置 → 云服务部署」接上个人云（和 Chrome 网页数据不互通），用 JWT 格式 service_role（eyJ 开头），再到聊天信息页创建定时/冷场规则。通知栏若停在请接上个人云，就是壳还没拿到地址；已连接却收不到测试推送时重新接上并升级云函数。",
  "",
  "## 环境变量速查（全部可选，除单机模式开关）",
