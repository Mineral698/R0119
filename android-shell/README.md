...
3. 跑完后在该次运行的 **Artifacts** 里下载：
   - `FLOAT` —— debug 签名的 `FLOAT.apk`，**下载即可直接安装**，日常自用选这个；
   - `FLOAT-release` —— 未配置签名密钥时是未签名包（装不了），配置后是正式签名包。
...
4. 保活（收推送的关键，尤其国产 ROM）：
   - 系统设置里把「FLOAT」加入**电池优化白名单 / 无限制后台**；
   - 允许**自启动**（小米/华为/OPPO/vivo 在各自的手机管家里）;
   - 最近任务里把它**锁定**（下拉卡片 → 锁），避免一键清理杀掉。
5. 通知栏会有一条「FLOAT · 后台连接」的常驻小通知——这是长连接保活的
   代价。连上后应显示「已连接，等待角色消息」。若一直是
   「请在本 App 打开设置→云服务部署，接上个人云」，说明壳还没拿到个人云地址：必须在
   **App 里**接上个人云（和 Chrome 网页不互通），不要只在浏览器里配置。
   通知栏已显示「已连接」却收不到测试推送时，用 JWT 格式 service_role（eyJ 开头）
   再接一次，并升级个人云函数。
   不想看到常驻通知可以长按它 → 把「后台连接」这个通知渠道关掉显示，
   **不要关「角色消息」渠道**。
...
- 自部署个人云：网页（以及壳在每次页面加载时直接读 WebView IndexedDB）把
  用户自己的 Supabase 地址交给 `configurePush`，壳直连该项目的 Realtime
  频道 `shellpush:owner`，并登记合成订阅 `shell:owner`。请安装 **1.1.2+**；
  1.0 只会去连站点联机库，自部署收不到离线消息。
  1.1.2 起：相同配置不再每 15 秒拆线重连；Realtime 公私频道都会订阅/广播，避免「已连接却收不到」。
