...
**设置 → 云服务部署**里粘贴自己的 Supabase Access Token（Classic Tokens，`sbp_` 开头）、选择自己的组织，应用会：
...
令牌读不到组织、提示「该项目没有可用的组织」时，可手填组织 slug，或改用 Classic Token / 「已经部署过」。
...
3. 安装 APK 后，**在 App 里**打开 **设置 → 云服务部署** 接上个人云（Chrome 网页里配过不算，数据不互通）；已经建过的用「已经部署过」；
4. 聊天信息页创建「长时间没消息时」或「固定时间后」规则，并允许系统通知。

请安装 **1.1.2+** 的壳。通知栏若停在「请在本 App 打开设置→云服务部署」，就是壳还没拿到个人云地址。旧版 1.0 只会去连站点联机库。通知栏已显示「已连接」却收不到测试推送时：在 App 里用 JWT 格式的 service_role（`eyJ` 开头）重新点「接上已有个人云」，并重新部署一次个人云函数。
